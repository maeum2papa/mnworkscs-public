<script>
    ...
    const column = [
        ...
        },
        {
            name : '^제품증상',
            use : true,
            columnName : 'symptomFl',
            dataType:'s'
        },
        {
            name : '^증상상세',
            use : true,
            columnName : 'symptomDetail',
            dataType:'s'
        },
        {
            name : '^주소',
        ...
        },
        {
            name : '^입고송장번호',
            use : true,
            columnName : 'recInvoice',
            dataType:'s'
        },
        {
            name : '^출고송장번호',
            use : true,
            columnName : 'forInvoice',
            dataType:'s'
        },
    ]
...
<div class="hidden">
    <table bind:this={table}>
        ...
        {#each asResult as lietItem,i}
        <tr>
            {#each column as item}
                {#if item.use}
                    <td>^{#if item.columnName=='buyDate' || item.columnName=='regDate' }
                            {dateFormat(lietItem[item.columnName])}
                        {:else if item.columnName == 'symptomFl'}
                            {category[Number(lietItem[item.columnName])-1]} 
                        {:else}
                            {lietItem[item.columnName]}
                        {/if}
                        <!-- {#if item.columnName == 'symptomFl'}
                        {/if} -->
                    </td>
                {/if}
            {/each}
